...
class AssetsHistoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = AssetsHistory::with(['asset', 'status', 'location', 'assigned_user', 'created_by'])->select(sprintf('%s.*', (new AssetsHistory())->table));
    ...
    public function show(AssetsHistory $assetsHistory)
    {
        ...
        $assetsHistory->load('asset', 'status', 'location', 'assigned_user', 'created_by');
