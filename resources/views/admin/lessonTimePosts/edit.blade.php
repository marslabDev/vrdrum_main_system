...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.lessonTimePost.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.lesson-time-posts.update", [$lessonTimePost->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="group">{{ trans('cruds.lessonTimePost.fields.group') }}</label>
                <input class="form-control {{ $errors->has('group') ? 'is-invalid' : '' }}" type="text" name="group" id="group" value="{{ old('group', $lessonTimePost->group) }}" required>
                @if($errors->has('group'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category" id="category" required>
                    ...
                    @foreach(App\Models\LessonTimePost::CATEGORY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('category', $lessonTimePost->category) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $lessonTimePost->title) }}" required>
                @if($errors->has('title'))
                    <div class="invalid-feedback">
                        {{ $errors->first('title') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('desc') ? 'is-invalid' : '' }}" name="desc" id="desc">{!! old('desc', $lessonTimePost->desc) !!}</textarea>
                @if($errors->has('desc'))
                    <div class="invalid-feedback">
                        {{ $errors->first('desc') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('publish_at') ? 'is-invalid' : '' }}" type="text" name="publish_at" id="publish_at" value="{{ old('publish_at', $lessonTimePost->publish_at) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('terminate_at') ? 'is-invalid' : '' }}" type="text" name="terminate_at" id="terminate_at" value="{{ old('terminate_at', $lessonTimePost->terminate_at) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('start_at') ? 'is-invalid' : '' }}" type="text" name="start_at" id="start_at" value="{{ old('start_at', $lessonTimePost->start_at) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('end_at') ? 'is-invalid' : '' }}" type="text" name="end_at" id="end_at" value="{{ old('end_at', $lessonTimePost->end_at) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('required_response') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="required_response" id="required_response" value="1" {{ $lessonTimePost->required_response || old('required_response', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lesson_time') ? 'is-invalid' : '' }}" name="lesson_time_id" id="lesson_time_id" required>
                    @foreach($lesson_times as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lesson_time_id') ? old('lesson_time_id') : $lessonTimePost->lesson_time->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
