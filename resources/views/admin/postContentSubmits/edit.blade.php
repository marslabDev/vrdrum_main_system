...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.postContentSubmit.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.post-content-submits.update", [$postContentSubmit->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="title">{{ trans('cruds.postContentSubmit.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $postContentSubmit->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('desc') ? 'is-invalid' : '' }}" name="desc" id="desc">{!! old('desc', $postContentSubmit->desc) !!}</textarea>
                @if($errors->has('desc'))
                    <div class="invalid-feedback">
                        {{ $errors->first('desc') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('mark') ? 'is-invalid' : '' }}" type="number" name="mark" id="mark" value="{{ old('mark', $postContentSubmit->mark) }}" step="0.01">
                @if($errors->has('mark'))
                    <div class="invalid-feedback">
                        {{ $errors->first('mark') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('objective_answers') ? 'is-invalid' : '' }}" type="text" name="objective_answers" id="objective_answers" value="{{ old('objective_answers', $postContentSubmit->objective_answers) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('post_content') ? 'is-invalid' : '' }}" name="post_content_id" id="post_content_id" required>
                    @foreach($post_contents as $id => $entry)
                        <option value="{{ $id }}" {{ (old('post_content_id') ? old('post_content_id') : $postContentSubmit->post_content->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
