...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.coachDetail.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.coach-details.update", [$coachDetail->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('enrollment_status') ? 'is-invalid' : '' }}" name="enrollment_status" id="enrollment_status" required>
                    ...
                    @foreach(App\Models\CoachDetail::ENROLLMENT_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('enrollment_status', $coachDetail->enrollment_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('coach_efk') ? 'is-invalid' : '' }}" type="number" name="coach_efk" id="coach_efk" value="{{ old('coach_efk', $coachDetail->coach_efk) }}" step="1" required>
