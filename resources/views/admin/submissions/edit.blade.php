    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.submissions.update", [$submission->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\Submission::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $submission->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('submit_at') ? 'is-invalid' : '' }}" type="text" name="submit_at" id="submit_at" value="{{ old('submit_at', $submission->submit_at) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('mark') ? 'is-invalid' : '' }}" type="number" name="mark" id="mark" value="{{ old('mark', $submission->mark) }}" step="0.01">
                @if($errors->has('mark'))
                    <div class="invalid-feedback">
                        {{ $errors->first('mark') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('mark_at') ? 'is-invalid' : '' }}" type="text" name="mark_at" id="mark_at" value="{{ old('mark_at', $submission->mark_at) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('student_work') ? 'is-invalid' : '' }}" name="student_work_id" id="student_work_id">
                    @foreach($student_works as $id => $entry)
                        <option value="{{ $id }}" {{ (old('student_work_id') ? old('student_work_id') : $submission->student_work->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
