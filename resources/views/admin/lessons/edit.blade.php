    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.lessons.update", [$lesson->id]) }}" enctype="multipart/form-data">
            ...
            <!-- <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_of_class') ? 'is-invalid' : '' }}" type="number" name="no_of_class" id="no_of_class" value="{{ old('no_of_class', $lesson->no_of_class) }}" step="1" disabled>
            </div> -->
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('coachs_efk[]') ? 'is-invalid' : '' }}" name="coachs_efk[]" id="coachs_efk[]" multiple="multiple">
                    @foreach($coachs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('coachs_efk[]') ? old('coachs_efk[]') : $old_coachs_efk[$entry] ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
