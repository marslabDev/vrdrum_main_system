...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.studentWork.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.student-works.update", [$studentWork->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category" id="category">
                    ...
                    @foreach(App\Models\StudentWork::CATEGORY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('category', $studentWork->category) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $studentWork->title) }}" required>
                @if($errors->has('title'))
                    <div class="invalid-feedback">
                        {{ $errors->first('title') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('desc') ? 'is-invalid' : '' }}" type="text" name="desc" id="desc" value="{{ old('desc', $studentWork->desc) }}">
                @if($errors->has('desc'))
                    <div class="invalid-feedback">
                        {{ $errors->first('desc') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('start_at') ? 'is-invalid' : '' }}" type="text" name="start_at" id="start_at" value="{{ old('start_at', $studentWork->start_at) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('end_at') ? 'is-invalid' : '' }}" type="text" name="end_at" id="end_at" value="{{ old('end_at', $studentWork->end_at) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('time_given_minute') ? 'is-invalid' : '' }}" type="number" name="time_given_minute" id="time_given_minute" value="{{ old('time_given_minute', $studentWork->time_given_minute) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lesson_time') ? 'is-invalid' : '' }}" name="lesson_time_id" id="lesson_time_id">
                    @foreach($lesson_times as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lesson_time_id') ? old('lesson_time_id') : $studentWork->lesson_time->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
