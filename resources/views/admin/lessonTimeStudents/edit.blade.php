...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.lessonTimeStudent.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.lesson-time-students.update", [$lessonTimeStudent->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="attended_at">{{ trans('cruds.lessonTimeStudent.fields.attended_at') }}</label>
                <input class="form-control datetime {{ $errors->has('attended_at') ? 'is-invalid' : '' }}" type="text" name="attended_at" id="attended_at" value="{{ old('attended_at', $lessonTimeStudent->attended_at) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('leave_at') ? 'is-invalid' : '' }}" type="text" name="leave_at" id="leave_at" value="{{ old('leave_at', $lessonTimeStudent->leave_at) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('cancel_at') ? 'is-invalid' : '' }}" type="text" name="cancel_at" id="cancel_at" value="{{ old('cancel_at', $lessonTimeStudent->cancel_at) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lesson_time') ? 'is-invalid' : '' }}" name="lesson_time_id" id="lesson_time_id">
                    @foreach($lesson_times as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lesson_time_id') ? old('lesson_time_id') : $lessonTimeStudent->lesson_time->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('student_efk') ? 'is-invalid' : '' }}" type="number" name="student_efk" id="student_efk" value="{{ old('student_efk', $lessonTimeStudent->student_efk) }}" step="1" required>
