...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.studentDetail.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.student-details.update", [$studentDetail->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="full_name">{{ trans('cruds.studentDetail.fields.full_name') }}</label>
                <input class="form-control {{ $errors->has('full_name') ? 'is-invalid' : '' }}" type="text" name="full_name" id="full_name" value="{{ old('full_name', $studentDetail->full_name) }}" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_handicapped') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_handicapped" id="is_handicapped" value="1" {{ $studentDetail->is_handicapped || old('is_handicapped', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('gender') ? 'is-invalid' : '' }}" name="gender" id="gender">
                    ...
                    @foreach(App\Models\StudentDetail::GENDER_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('gender', $studentDetail->gender) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('country') ? 'is-invalid' : '' }}" type="text" name="country" id="country" value="{{ old('country', $studentDetail->country) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('home_address') ? 'is-invalid' : '' }}" type="text" name="home_address" id="home_address" value="{{ old('home_address', $studentDetail->home_address) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('mail_address') ? 'is-invalid' : '' }}" type="text" name="mail_address" id="mail_address" value="{{ old('mail_address', $studentDetail->mail_address) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('state') ? 'is-invalid' : '' }}" type="text" name="state" id="state" value="{{ old('state', $studentDetail->state) }}">
                @if($errors->has('state'))
                    <div class="invalid-feedback">
                        {{ $errors->first('state') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('city') ? 'is-invalid' : '' }}" type="text" name="city" id="city" value="{{ old('city', $studentDetail->city) }}">
                @if($errors->has('city'))
                    <div class="invalid-feedback">
                        {{ $errors->first('city') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('postcode') ? 'is-invalid' : '' }}" type="number" name="postcode" id="postcode" value="{{ old('postcode', $studentDetail->postcode) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('phone') ? 'is-invalid' : '' }}" type="text" name="phone" id="phone" value="{{ old('phone', $studentDetail->phone) }}">
                @if($errors->has('phone'))
                    <div class="invalid-feedback">
                        {{ $errors->first('phone') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nric_no') ? 'is-invalid' : '' }}" type="number" name="nric_no" id="nric_no" value="{{ old('nric_no', $studentDetail->nric_no) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $studentDetail->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('guardians') ? 'is-invalid' : '' }}" name="guardians[]" id="guardians" multiple>
                    @foreach($guardians as $id => $guardian)
                        <option value="{{ $id }}" {{ (in_array($id, old('guardians', [])) || $studentDetail->guardians->contains($id)) ? 'selected' : '' }}>{{ $guardian }}</option>
                    @endforeach
                </select>
