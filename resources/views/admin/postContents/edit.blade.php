...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.postContent.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.post-contents.update", [$postContent->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('resource_type') ? 'is-invalid' : '' }}" name="resource_type" id="resource_type" required>
                    ...
                    @foreach(App\Models\PostContent::RESOURCE_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('resource_type', $postContent->resource_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('submit_type') ? 'is-invalid' : '' }}" name="submit_type" id="submit_type">
                    ...
                    @foreach(App\Models\PostContent::SUBMIT_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('submit_type', $postContent->submit_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $postContent->title) }}" required>
                @if($errors->has('title'))
                    <div class="invalid-feedback">
                        {{ $errors->first('title') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('desc') ? 'is-invalid' : '' }}" name="desc" id="desc">{!! old('desc', $postContent->desc) !!}</textarea>
                @if($errors->has('desc'))
                    <div class="invalid-feedback">
                        {{ $errors->first('desc') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('mark') ? 'is-invalid' : '' }}" type="number" name="mark" id="mark" value="{{ old('mark', $postContent->mark) }}" step="0.01">
                @if($errors->has('mark'))
                    <div class="invalid-feedback">
                        {{ $errors->first('mark') }}
                    </div>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('required_response') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="required_response" id="required_response" value="1" {{ $postContent->required_response || old('required_response', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('objective_selections') ? 'is-invalid' : '' }}" type="text" name="objective_selections" id="objective_selections" value="{{ old('objective_selections', $postContent->objective_selections) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('objective_answers') ? 'is-invalid' : '' }}" type="text" name="objective_answers" id="objective_answers" value="{{ old('objective_answers', $postContent->objective_answers) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lesson_time_post') ? 'is-invalid' : '' }}" name="lesson_time_post_id" id="lesson_time_post_id" required>
                    @foreach($lesson_time_posts as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lesson_time_post_id') ? old('lesson_time_post_id') : $postContent->lesson_time_post->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
