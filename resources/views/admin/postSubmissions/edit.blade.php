...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.postSubmission.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.post-submissions.update", [$postSubmission->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="status">{{ trans('cruds.postSubmission.fields.status') }}</label>
                <input class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" type="text" name="status" id="status" value="{{ old('status', $postSubmission->status) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('submit_at') ? 'is-invalid' : '' }}" type="text" name="submit_at" id="submit_at" value="{{ old('submit_at', $postSubmission->submit_at) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('mark') ? 'is-invalid' : '' }}" type="number" name="mark" id="mark" value="{{ old('mark', $postSubmission->mark) }}" step="0.01">
                @if($errors->has('mark'))
                    <div class="invalid-feedback">
                        {{ $errors->first('mark') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('mark_at') ? 'is-invalid' : '' }}" type="text" name="mark_at" id="mark_at" value="{{ old('mark_at', $postSubmission->mark_at) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lesson_time_post') ? 'is-invalid' : '' }}" name="lesson_time_post_id" id="lesson_time_post_id" required>
                    @foreach($lesson_time_posts as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lesson_time_post_id') ? old('lesson_time_post_id') : $postSubmission->lesson_time_post->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('student_efk') ? 'is-invalid' : '' }}" type="number" name="student_efk" id="student_efk" value="{{ old('student_efk', $postSubmission->student_efk) }}" step="1" required>
