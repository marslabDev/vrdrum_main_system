    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.post-comments.update", [$postComment->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('content') ? 'is-invalid' : '' }}" name="content" id="content">{!! old('content', $postComment->content) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lesson_time_post') ? 'is-invalid' : '' }}" name="lesson_time_post_id" id="lesson_time_post_id" required>
                    @foreach($lesson_time_posts as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lesson_time_post_id') ? old('lesson_time_post_id') : $postComment->lesson_time_post->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sender_efk') ? 'is-invalid' : '' }}" type="number" name="sender_efk" id="sender_efk" value="{{ old('sender_efk', $postComment->sender_efk) }}" step="1">
